...
| **증거계약 배치 — 토대유닛 오부착** *(#126 발견)* | 엔진-trace 증거계약이 `deps=[]` 스캐폴드 유닛에 떨어짐 → 엔진(다운스트림)이 아직 없어 충족 불가 → 토대유닛 retry 낭비(캡스톤 #126 platformer u0: 5 dispatch[retry 0–3 medium→high→xhigh + OR-대안], OR이 흡수해 자가교정·escalate 0) | 증거계약 **배치를 dep-순서 인식**하게(엔진-trace 계약은 엔진 유닛으로, 스캐폴드 아님) / 스캐폴드 trace = no-op 표면 |
| **브라우저-하니스 vs 샌드박스 loopback EPERM** *(#126 발견 — "플랫포머 onset")* | 게임플레이 하니스가 127.0.0.1 서버(headless Chrome/CDP)를 호스팅해 실브라우저 플레이스루를 관측하려다 샌드박스가 loopback listen을 **EPERM** 차단 → `trace:browser-render` 통합 fail(게임 hollow 아님 — in-sandbox engine-trace는 전체 행동 증거 생산; 브라우저 레이어만 막힘. ALLOWED_SANDBOXES 불변·올바름) | **server-less 검증** — in-sandbox engine-trace(서버 불요·전체 행동 증거)를 행동 권위로 + render는 file://·data-URI/best-effort. **ALLOWED_SANDBOXES 변경 금지(#84 안전 불변 강화 — loopback 허용 안 함)** |
