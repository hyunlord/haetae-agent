...
- gap이 *동적/런타임/시각* 성격(움직임·실시간 갱신·에이전트 행동·애니메이션 등)이면,
  `strengthening`으로 **가능하면 `run` 타입 기준(헤드리스 트레이스 진입점)을 권장**하라 —
  "사람이 수동으로 확인"만 권하지 말 것. run-judge가 그 트레이스로 동적 행동을 *자동* 판정할
  수 있으니, 자동 종료 가능한 강화가 더 강한 비평이다.
