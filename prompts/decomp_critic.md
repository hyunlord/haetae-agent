...
3. **직전 실패 반복** — 직전 진행(last_result)에서 실패/거부된 접근을 그대로 다시 시도.
   같은 자리를 맴돎.
4. **과대-다행동 유닛 (입도/책임 수 축)** — 한 유닛이 *서로 독립인 행동 여럿*(예: 이동 +
   충돌 + 먹이 + 점수 + game-over)을 한 덩어리로 묶고 한 유닛 테스트가 그 다수 행동을 다
   ...
   **과대**다 → `weak`로 플래그하고, 각 독립 행동을 *distinct 모듈 파일을 소유하는 단일-책임
   ...
   단, **한 책임의 하위측면**(예: "충돌 = 벽·자기몸")을 묶은 건 과대가 아니다(과분할 금지).
   통합/조립 유닛이 모듈 다수를 wire하는 건 정상이다(그 유닛은 조립 파일만 소유 — 면제).
   입력에 `입도 신호`(자동 탐지)가 붙어오면 *참고*하되, 최종 판정은 행동의 실제 독립성으로 하라.
