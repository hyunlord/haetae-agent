...
- **단일-책임 + disjoint-scope 분해 (입도 — 중요).** 한 유닛에 *서로 독립인 행동 여럿*을 몰지
  마라. 약한 빌더는 다행동 단일 유닛을 신뢰성있게 수렴하지 못한다(#147: snake 엔진의
  이동+성장+충돌+먹이+점수+game-over를 한 유닛에 몰자 미수렴). 규칙: **독립 행동 하나 = 유닛
  하나**, 각자 *distinct 모듈 파일*을 `scope`로 소유(예: `engine/move.js`·`collision.js`·
  `food.js`·`gameover.js`)하고 *자체 작은 테스트*를 가진다. scope는 형제 간 **겹침 0**
  (disjoint — 머지/통합 벽 악화 방지, #51/#123 일관). 그리고 그 모듈들을 조립하는 **통합/조립
  유닛**(예: `engine.js`가 모듈들을 wire, `unit: integration` 기준)을 따로 둬라. 한 유닛이
  4개 이상 독립 행동을 묶으면 과대다 — 쪼개라. (단, *한 책임의 하위측면*인 "충돌=벽·자기몸"을
  한 유닛에 두는 건 정상 — 과분할 금지.)
